  ...
  const courses = [
    {
      ...
      description: "50+ web courses available",
    },
    {
      id: "Marketing",
      img: png2,
  ...
  return (
    <div>
      <div className="flex flex-col justify-center m-auto text-center py-10 px-4 mt-24">
        ...
        <span className="mb-14 sm:text-[0.95em] text-[0.8em]">
          Explore a variety of courses taught by expert mentors
        </span>
        ...
        <div className="m-auto grid lg:gap-6 xl:gap-24 gap-24 lg:grid-cols-3 md:grid-cols-2 grid-cols-1 items-center">
          {courses.map((course) => (
            <div
              key={course.id}
              id={course.id}
              className="flex gap-5 text-start items-center"
            >
              <Image
                src={course.img}
                alt={course.alt}
                className="sm:w-[70px] w-[45px] h-auto"
              />
              <div>
                <h3 className="sm:text-xl text-lg font-semibold">
                  {course.title}
                </h3>
                <span className="sm:text-[0.95em] text-[0.85em] tracking-wide">
                  {course.description}
                </span>
              </div>
            </div>
